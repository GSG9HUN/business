...
- `Commands/TextCommands/Playlist/` contains one test file per playlist command plus a shared test base, including `LoadPlaylistCommandTests.cs` for saved playlist queue loading and idle playback start.
- `Commands/SlashCommands/Playlist/PlaylistSlashCommandTests.cs` covers playlist slash adapter delegation, quoted playlist-name payloads, deferred save/load/add-song requests, and delete confirmation.
