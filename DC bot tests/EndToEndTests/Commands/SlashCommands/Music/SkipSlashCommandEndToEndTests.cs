...
[Trait("Category", "E2E")]
public class SkipSlashCommandEndToEndTests : SlashCommandPipelineEndToEndTestBase
{
    [Fact]
    public async Task SkipSlashCommand_ShouldRunThroughTextCommandPipeline()
    {
        var member = CreateMember("SlashUser", "<@123>", CreateVoiceChannel());
        var context = CreateContext(member: member);
        var command = new SkipSlashCommand(SlashCommandExecutor, Mock.Of<ISlashInteractionContextFactory>());
