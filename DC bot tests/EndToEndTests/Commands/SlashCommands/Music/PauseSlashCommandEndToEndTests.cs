...
[Trait("Category", "E2E")]
public class PauseSlashCommandEndToEndTests : SlashCommandPipelineEndToEndTestBase
{
    [Fact]
    public async Task PauseSlashCommand_ShouldRunThroughTextCommandPipeline()
    {
        var member = CreateMember("SlashUser", "<@123>", CreateVoiceChannel());
        var context = CreateContext(member: member);
        var command = new PauseSlashCommand(SlashCommandExecutor, Mock.Of<ISlashInteractionContextFactory>());
